...
type: IDENT "<" type ">"  -> paramtype
    | IDENT               -> primtype
    | IDENT "<" type ">," IDENT "<" type ">" -> doubleparamtype
...
class JSONTransformer(lark.Transformer):
    ...
    def doubleparamtype(self, items):
...
def type_to_str(type):
    if isinstance(type, dict):
        assert len(type) <= 2
        # Only one key-value pair (pointer type).
        if len(type) == 1:
            key, value = next(iter(type.items()))
            return '{}<{}>'.format(key, type_to_str(value))
        elif len(type) == 2:
            types_iter = iter(type.items())
            key1, value1 = next(types_iter)
            key2, value2 = next(types_iter)
            return '{}<{}>, {}<{}>'.format(key1, type_to_str(value1), key2, type_to_str(value2))
    else:
        return type
